...
struct abcb {
#define aaa(type, name) type name
	aaa(long long int, acd);
#undef aaa
};

#define VLIB_REGISTER_NODE(x, ...) __VA_ARGS__ hds_pip_info_t x

VLIB_REGISTER_NODE(regisger_node) = {
	.ip   = 1234,
	.time = 5678,
};
int main(void)
{
	...
	struct abcb abcd, efgh;

	abcd.acd = 123;

	efgh = abcd;

	printf("ip:%u  time:%u\n", regisger_node.ip, regisger_node.time);
	//printf("abcd:%lu sizeof(abc):%lu\n", sizeof(abcd), sizeof(struct abc));
	printf("abcd:%lu efgh:%d sizeof(abc):%lu\n", sizeof(abcd), efgh.acd, sizeof(struct abc));
	// 结构体赋值
	*p = b;
	printf("parent:%p i:%p color:%d right:%p k:%p \n", p->parent, &i, p->color, p->right, &k);
	for (i = 0; i < 0; i++)
		;
	printf("%d pack:%lu\n", i, sizeof(hds_k5v_t));
